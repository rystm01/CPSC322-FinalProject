        ...
        [
            ...
            [
                ...
                [
                    "Value",
                    1,
                    ["Attribute", "att2", ["Value", "fair", ["Leaf", "yes", 1, 1]]],
                ],
                [
                    "Value",
                    2,
            ...
            [
                ...
                [
                    ...
                    [
                        "Attribute",
                        "att2",
                        ["Value", "excellent", ["Leaf", "yes", 2, 3]],
                        ["Value", "fair", ["Leaf", "yes", 2, 3]],
                    ],
                ],
                [
                    "Value",
...
def test_decision_tree_classifier_predict():
    ...
    X_test_false = [["Junior", "Java", "yes", "yes"], ["Senior", "Java", "no", "no"]]
...
def test_kneighbors_classifier_kneighbors():
    ...
    X_train_scaled = [
        [(x[0] - x1_min) / (x1_max - x1_min), (x[1] - x2_min) / (x2_max - x2_min)]
        for x in X_train
    ]
    X_test_scaled = [
        [(x[0] - x1_min) / (x1_max - x1_min), (x[1] - x2_min) / (x2_max - x2_min)]
    ...
    assert np.isclose(np.round(distances[0][0], 3), 0.608)
...
def test_dummy_classifier_fit():
    ...
    y_train = list(
        np.random.choice(
            ["yes", "no", "maybe", "no way"], 100, replace=True, p=[0.2, 0.1, 0.2, 0.5]
        )
    )
    my_dummy.fit([], y_train)
...
def test_naive_bayes_classifier_fit():
    ...
    for i in range(4):
        ...
        for list in X_list:
            for val in list:
                assert np.isclose(
                    posteriors[count], my_nb.posteriors[list_i][y_0[i]][val]
                )
                count += 1
            list_i += 1
